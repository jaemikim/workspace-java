 ...
 *    또한 여자일 경우에는 추가적인 정보 입력 대신에 "여성에게는 국방의 의무가 없습니다. "가 출력되게 만드시오.
 *    남자일 경우 미성년자 경우 추가적인 정보 입력 대신 "미성년자에게는 아직 신체등급이 부여되지 않습니다."가 출력되게 만드시오.
 *    
 *    - 예) 성별(1은 남자, 2는 여자):
 *         나이 : 20
 *         신체 등급 (1 ~ 4) : 1
	...
	public static void main(String[] args) {
		...
		if (gender == 1) {  
			...
			if(age >= 19) {
				...
				System.out.print("신체등급 입력하세요 : ");
				int grade = scan.nextInt();
				
				if(grade <= 3) {  //((grade >= 1 && grade <= 3))
					System.out.println("현역");
				} else if (grade == 4) {
					System.out.println("공익");
				} else {
					System.out.println("먼제입니다.");
				}
